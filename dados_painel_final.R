...
# extrato_fi_atual |> 
#   filter(!is.na(CLASSE_ANBIMA)) |> 
#   mutate(CLASSE_ANBIMA = str_trim(str_extract(CLASSE_ANBIMA,"\\w* \\w*"))) |> 
#   count(CLASSE_ANBIMA,INVEST_EXTERIOR) |> 
#   mutate(INVEST_EXTERIOR = case_when(INVEST_EXTERIOR == "N" ~ "Não",
#                                      T ~ "Sim")) |> 
#   mutate(`Part(%)` = n/sum(n),.by = "CLASSE_ANBIMA") |>
#   mutate(`Part(%)` = scales::percent(`Part(%)`)) |> 
#   rename(`Classe` = CLASSE_ANBIMA,
#          `Invest. Exterior?` = INVEST_EXTERIOR,
#          Qtd = n) |> 
#   kable(format = "latex",
#         caption = "Teste de Hausman - Modelos sem interações",
#         booktabs = T) |> 
...
plm(CAPTC_LIQ ~
    ...
    index = c("CNPJ_FUNDO","TRIMESTRE"),
    data=dados_painel_rf_trimestre_reg, 
    model="within") -> plm_rf_fixo_timef
...
plm(CAPTC_LIQ ~
    ...
    index = c("CNPJ_FUNDO","TRIMESTRE"),
    data=dados_painel_mm_trimestre_reg, 
    model="within") -> plm_mm_fixo_timef
...
plm(CAPTC_LIQ ~
    ...
    index = c("CNPJ_FUNDO","TRIMESTRE"),
    data=dados_painel_acoes_trimestre_reg, 
    model="within") -> plm_acoes_fixo_timef
...
plm(CAPTC_LIQ ~
    ...
    index = c("CNPJ_FUNDO","TRIMESTRE"),
    data=dados_painel_acoes_trimestre_reg, 
    model="random") -> plm_acoes_aleatorio

##### Testes Consistência #################

bind_rows(mutate(broom::tidy(phtest(plm_acoes_fixo,
                                    plm_acoes_aleatorio,
                                    model = c("within", "random"),
                                    vcov = vcovBK)),
                 Classe = "Ações"),
          mutate(broom::tidy(phtest(plm_mm_fixo,
                                    plm_mm_aleatorio,
                                    vcov = vcovBK)),
                 Classe = "Multimercado"),
          mutate(broom::tidy(phtest(plm_rf_fixo,
                                    plm_rf_aleatorio,
                                    vcov = vcovBK)),
                 Classe = "Renda Fixa")) |> 
  mutate(p.value = scales::number(p.value,
                                  decimal.mark = ",",
                                  accuracy = 0.0001),
         statistic = scales::number(statistic,
                                    decimal.mark = ",",
                                    big.mark = "\\.", 
                                    accuracy = 0.01)) |> 
...
bind_rows(mutate(tidy(pFtest(plm_acoes_fixo_timef,
                             plm_acoes_fixo)),
                 Classe = "Ações",
                 method = "Teste F para efeitos de tempo e indivíduos"),
          mutate(tidy(pFtest(plm_mm_fixo_timef,
                             plm_mm_fixo)),
                 Classe = "Multimercado",
                 method = "Teste F para efeitos de tempo e indivíduos"),
          mutate(tidy(pFtest(plm_rf_fixo_timef,
                             plm_rf_fixo)),
                 Classe = "Renda Fixa",
                 method = "Teste F para efeitos de tempo e indivíduos"),
          mutate(tidy(plmtest(plm_acoes_fixo,"time",type = "bp")),
                 Classe = "Ações",
                 method = "Teste de Multiplicadores de Lagrange - Efeitos de Tempo (BP)"),
          mutate(tidy(plmtest(plm_mm_fixo,"time",type = "bp")),
                 Classe = "Multimercado",
                 method = "Teste de Multiplicadores de Lagrange - Efeitos de Tempo (BP)"),
          mutate(tidy(plmtest(plm_rf_fixo,"time",type = "bp")),
                 Classe = "Renda Fixa",
                 method = "Teste de Multiplicadores de Lagrange - Efeitos de Tempo (BP)")
          ) |> 
  mutate(p.value = scales::number(p.value,
  ...
  rename(Estatística = statistic,
         `p valor` = p.value,
         Método = method) |> 
  select(-alternative,-df1,-df2,-parameter) |> 
  relocate(Classe) |> 
  arrange(Classe) |> 
  kable(format = "latex",
        caption = "Testes de efeitos fixos de tempo",
        booktabs = T) |> 
  kable_styling(latex_options = c("hold_position","scale_down"))


bind_rows(mutate(tidy(pcdtest(plm_rf_fixo, 
                              test = c("lm"))),
                 Classe = "Renda Fixa",
                 method = "Teste Breusch-Pagan LM - Dependência Cross-Sectional"),
          mutate(tidy(pcdtest(plm_rf_fixo, 
                              test = c("cd"))),
                 Classe = "Renda Fixa",
                 method = "Teste CD - Dependência Cross-Sectional"),
          mutate(tidy(pcdtest(plm_mm_fixo, 
                              test = c("lm"))),
                 Classe = "Multimercado",
                 method = "Teste Breusch-Pagan LM - Dependência Cross-Sectional"),
          mutate(tidy(pcdtest(plm_mm_fixo, 
                 ...
                 Classe = "Multimercado",
                 method = "Teste CD - Dependência Cross-Sectional"),
          mutate(tidy(pcdtest(plm_acoes_fixo, 
                              test = c("lm"))),
                 Classe = "Ações",
                 method = "Teste Breusch-Pagan LM - Dependência Cross-Sectional"),
          mutate(tidy(pcdtest(plm_acoes_fixo, 
  ...
  rename(Estatística = statistic,
         `p valor` = p.value,
         Método = method) |> 
  select(-alternative,-parameter) |> 
  relocate(Classe) |> 
  arrange(Classe) |> 
  kable(format = "latex",
        caption = "Testes de dependência transversal",
        booktabs = T) |> 
  kable_styling(latex_options = c("hold_position","scale_down"))


cov_acoes <- vcovBK(plm_acoes_fixo, 
                    type="HC3", 
                    cluster="time",
                    diagonal = T)

cov_mm <- vcovBK(plm_mm_fixo, 
                 type="HC3", 
                 cluster="time",
                 diagonal = T)

cov_rd <- vcovBK(plm_rf_fixo, 
                 type="HC3", 
                 cluster="time",
                 diagonal = T)

robust.se <- list(sqrt(diag(cov_acoes)),
                  sqrt(diag(cov_mm)),
                  sqrt(diag(cov_rd)))

stargazer(plm_acoes_fixo,
          plm_mm_fixo,
          plm_rf_fixo,
          se = robust.se,
          title="Modelos sem interações", 
          no.space = TRUE,
          align=TRUE,
          single.row = T, 
          column.sep.width = "-15pt",
          dep.var.caption = "Variável Dependente",
          column.labels = c("Ações","Multimercado","Renda Fixa") , 
          font.size = "tiny",
          decimal.mark = ",",
          digits = 2,
          dep.var.labels = "Captação Líquida",
          covariate.labels = c("CL_{t-1}",
                               "Ret",
                               "Ret_{12M}",
                               "TENDENCIA",
                               "IRF-M",
...
plm(CAPTC_LIQ ~
    ...
    index = c("CNPJ_FUNDO","TRIMESTRE"),
    data=dados_painel_rf_trimestre_reg, 
    model="pooling") -> plm_rf_aleatorio
...
bind_rows(mutate(broom::tidy(phtest(plm_acoes_fixo,
  ...
  relocate(Classe) |> 
  kable(format = "latex",
        caption = "Teste de Hausman - Modelos sem interações variáveis em log",
        booktabs = T) |> 
  kable_styling(latex_options = c("hold_position"))
...
bind_rows(mutate(tidy(pFtest(plm_acoes_fixo_timef,
  ...
  arrange(Classe) |> 
  kable(format = "latex",
        caption = "Testes de efeitos fixos de tempo variáveis em log",
        booktabs = T) |> 
  kable_styling(latex_options = c("hold_position","scale_down"))
...
bind_rows(mutate(tidy(pcdtest(plm_rf_fixo, 
  ...
  arrange(Classe) |> 
  kable(format = "latex",
        caption = "Testes de dependência transversal variáveis em log",
        booktabs = T) |> 
  kable_styling(latex_options = c("hold_position","scale_down"))
...
stargazer(plm_acoes_fixo,
          ...
          plm_rf_fixo,
          se = robust.se,
          title="Modelos sem interações variáveis em log", 
          no.space = TRUE,
          align=TRUE,
          single.row = T, 
          column.sep.width = "-15pt",
...
plm(CAPTC_LIQ ~
      lag(CAPTC_LIQ)+
      lag(RENTABILIDADE,1)*IQ+
      lag(RENTABILIDADE,1)*PG+
      lag(RENTABILIDADE,1)*GRANDES_BANCOS+
      lag(RENTABILIDADE,1)*GIGANTE+
      log(IRF_M)*PG+
      log(IMA_B)*PG+
      log(IMA_S)*PG+
      SELIC+
      ...
      TENDENCIA+
      TRIB_LPRAZO+
      RENTABILIDADE_ANO*PG*GRANDES_BANCOS+
      RENTABILIDADE_ANO*IQ*GRANDES_BANCOS+
      GIGANTE+
      GRANDE+
      MÉDIO,
    index = c("CNPJ_FUNDO","TRIMESTRE"),
...
bind_rows(mutate(broom::tidy(phtest(plm_acoes_fixo,
  ...
  relocate(Classe) |> 
  kable(format = "latex",
        caption = "Teste de Hausman - Modelos com interações",
        booktabs = T) |> 
  kable_styling(latex_options = c("hold_position"))
...
bind_rows(mutate(tidy(pFtest(plm_acoes_fixo_timef,
  ...
  arrange(Classe) |> 
  kable(format = "latex",
        caption = "Testes de efeitos fixos de tempo - modelos com interações",
        booktabs = T) |> 
  kable_styling(latex_options = c("hold_position","scale_down"))
...
bind_rows(mutate(tidy(pcdtest(plm_rf_fixo, 
  ...
  arrange(Classe) |> 
  kable(format = "latex",
        caption = "Testes de dependência transversal - modelos com interações",
        booktabs = T) |> 
  kable_styling(latex_options = c("hold_position","scale_down"))
...
bind_rows(mutate(broom::tidy(phtest(plm_acoes_fixo,
  ...
  relocate(Classe) |> 
  kable(format = "latex",
        caption = "Teste de Hausman - Modelos com interações variáveis em \textit{log}",
        booktabs = T) |> 
  kable_styling(latex_options = c("hold_position"))
...
bind_rows(mutate(tidy(pFtest(plm_acoes_fixo_timef,
  ...
  arrange(Classe) |> 
  kable(format = "latex",
        caption = "Testes de efeitos fixos de tempo - modelos com interações variáveis em log",
        booktabs = T) |> 
  kable_styling(latex_options = c("hold_position","scale_down"))
...
bind_rows(mutate(tidy(pcdtest(plm_rf_fixo, 
  ...
  arrange(Classe) |> 
  kable(format = "latex",
        caption = "Testes de dependência transversal - modelos com interações variáveis em log",
        booktabs = T) |> 
  kable_styling(latex_options = c("hold_position","scale_down"))
...
stargazer(plm_acoes_fixo,
          ...
          plm_rf_fixo,
          se = robust.se,
          title="Modelos com interações logaritimos", 
          no.space = TRUE,
          align=TRUE,
          single.row = T, 
          column.sep.width = "-15pt",
