...
dados_painel_rf |> 
  filter(CNPJ_FUNDO == "00.068.305/0001-35") |> View()
...
dados_painel_rf |> 
  ...
  group_by(CNPJ_FUNDO) |>
  mutate(RETORNO = round(((VL_QUOTA/dplyr::lag(VL_QUOTA))-1),5),
         CAPTC_LIQ = (CAPTC_DIA-RESG_DIA)/1000000) |> 
  mutate(RETORNO = case_when(is.na(RETORNO) ~ 0,
                             T ~ RETORNO)) |> 
  mutate(RETORNO_L1 = dplyr::lag(RETORNO),
         RETORNO_L2 = dplyr::lag(RETORNO,2)) |> 
  mutate(CAPTC_LIQ_ACUM = cumsum(CAPTC_LIQ),
         RETORNO_ACUM = cumprod(RETORNO+1)) |> 
  collect() -> dados_painel_rf_mensal

dados_painel_rf_mensal |>  
  ungroup() |> 
  select(MÊS,
         CNPJ_FUNDO,
         CAPTC_DIA,
         ...
         CAPTC_LIQ_ACUM,
         RETORNO_ACUM) |> 
  ggplot(aes(CAPTC_LIQ_ACUM,RETORNO_ACUM)) + 
  geom_point()
